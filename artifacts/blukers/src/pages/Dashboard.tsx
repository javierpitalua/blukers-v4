...
import {
  Briefcase, Users, CalendarClock, FileCheck, TrendingUp, TrendingDown,
  Plus, Search, GitBranch, ArrowRight, Clock, MoreVertical
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const activities = [
    { id: 1, title: "Carlos Mendez moved to Skills Assessment", time: "2 hours ago", color: "bg-purple-500" },
    { id: 2, title: "New application from James Wilson", time: "4 hours ago", color: "bg-blue-500" },
    { id: 3, title: "Job posted: Crane Operator", time: "1 day ago", color: "bg-emerald-500" },
    { id: 4, title: "Offer accepted by Robert Davis", time: "2 days ago", color: "bg-green-500" },
  ];
    ...
    <AppLayout>
      <div className="space-y-8 max-w-[1600px] mx-auto pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: "Active Jobs", value: "4", icon: Briefcase, iconBg: "bg-blue-50", iconColor: "text-blue-600", trend: "+12%", trendLabel: "from last month", up: true },
            { label: "Total Candidates", value: "8", icon: Users, iconBg: "bg-indigo-50", iconColor: "text-indigo-600", trend: "+24%", trendLabel: "from last month", up: true },
            { label: "Pending Interviews", value: "2", icon: CalendarClock, iconBg: "bg-amber-50", iconColor: "text-amber-600", trend: "-1", trendLabel: "from last week", up: false },
            { label: "Offers Extended", value: "1", icon: FileCheck, iconBg: "bg-emerald-50", iconColor: "text-emerald-600", trend: "+100%", trendLabel: "acceptance rate", up: true },
          ].map((m) => (
            <Card key={m.label} className="border-none shadow-sm bg-white hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider">{m.label}</p>
                    <p className="text-4xl font-bold text-slate-900">{m.value}</p>
                  </div>
                  <div className={`p-3 ${m.iconBg} rounded-xl`}>
                    <m.icon className={`w-6 h-6 ${m.iconColor}`} />
                  </div>
                </div>
                <div className="mt-6 flex items-center text-sm bg-slate-50 p-2 rounded-lg">
                  {m.up ? <TrendingUp className="w-4 h-4 text-emerald-500 mr-1.5" /> : <TrendingDown className="w-4 h-4 text-red-500 mr-1.5" />}
                  <span className={`${m.up ? "text-emerald-600" : "text-red-600"} font-semibold mr-1.5`}>{m.trend}</span>
                  <span className="text-slate-500">{m.trendLabel}</span>
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Link href="/jobs">
              <Button variant="outline" className="h-auto p-6 justify-start group hover:border-blue-300 hover:bg-blue-50/50 bg-white border-slate-200 rounded-2xl shadow-sm transition-all w-full">
            ...
            <Link href="/candidates">
              <Button variant="outline" className="h-auto p-6 justify-start group hover:border-indigo-300 hover:bg-indigo-50/50 bg-white border-slate-200 rounded-2xl shadow-sm transition-all w-full">
            ...
            <Link href="/pipeline">
              <Button variant="outline" className="h-auto p-6 justify-start group hover:border-purple-300 hover:bg-purple-50/50 bg-white border-slate-200 rounded-2xl shadow-sm transition-all w-full">
              ...
              <div className="relative pl-6 space-y-8 before:absolute before:inset-0 before:ml-[1.45rem] before:-translate-x-px before:h-full before:w-[2px] before:bg-slate-100">
                {activities.map((a) => (
                  <div key={a.id} className="relative flex items-start gap-5">
                    <div className={`absolute -left-6 w-3 h-3 rounded-full mt-1.5 border-2 border-white ring-4 ring-white ${a.color} shadow-sm z-10`} />
                    <div className="flex-1">
                      <p className="text-[15px] font-medium text-slate-900 leading-tight">{a.title}</p>
